...
function PhotoUpload({ workId, onUpload }) {
  ...
  return (
    <form onSubmit={handleUpload} className="bg-white border border-gray-200 rounded-lg p-5">
      ...
      <input type="file" accept="image/*" onChange={(event) => setFile(event.target.files?.[0] || null)} disabled={uploading || result} className="text-sm mb-4 w-full" />
      ...
      {result && <div className="bg-green-50 border border-green-200 rounded p-3 mt-3"><p className="text-sm font-semibold text-green-700">{result.status}</p><p className="text-xs text-gray-500">Verification score: {result.verification_score}</p></div>}
    </form>
  );
}
