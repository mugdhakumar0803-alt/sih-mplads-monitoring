...
def _score(components: dict) -> Optional[float]:
    ...
    if not available:
        return None
    weights = {"completion": 0.25, "timeliness": 0.20, "utilization": 0.20, "grievance_resolution": 0.15, "compliance": 0.10, "risk_resolution": 0.10}
    total_weight = sum(weights[name] for name in available)
    return round(sum(available[name] * weights[name] for name in available) / total_weight, 1)
...
def _work_metrics(db: Session, works: list[Work]) -> dict:
    ...
    assessed = sum(work.risk_level is not None for work in works)
    with_expenditure = [work for work in works if work.expenditure_amount is not None and work.allocation_amount]
    utilization = (sum(work.expenditure_amount for work in with_expenditure) / sum(work.allocation_amount for work in with_expenditure) * 100) if with_expenditure else None
    components = {
        "completion": completed / total * 100 if total else None,
        "timeliness": None,
        "utilization": utilization,
...
@router.get("")
async def mp_performance(house: Optional[str] = Query(None), state: Optional[str] = Query(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    groups = defaultdict(list)
    for work in db.query(Work).all():
        if state and work.state != state:
            continue
    ...
    for (mp_name, work_state, constituency), works in groups.items():
        ...
        ratings = db.query(Rating).join(Work, Rating.work_id == Work.id).filter(Work.mp_name == mp_name).all()
        if ratings:
            metrics["citizen_rating"] = round(sum(rating.overall_score for rating in ratings) / len(ratings), 2)
            metrics["citizen_rating_count"] = len(ratings)
        else:
            metrics["citizen_rating"] = None
            metrics["citizen_rating_count"] = 0
        rows.append({"mp_name": mp_name, "house": house or "Data unavailable", "state": work_state, "constituency": constituency, **metrics})
    rows.sort(key=lambda row: row["score"] if row["score"] is not None else -1, reverse=True)
    for rank, row in enumerate(rows, 1):
        row["rank"] = rank
    return {"type": "mp", "weights": {"completion": 25, "timeliness": 20, "utilization": 20, "grievance_resolution": 15, "compliance": 10, "risk_resolution": 10}, "leaderboard": rows}
...
@router.get("/districts")
async def district_performance(state: Optional[str] = Query(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    groups = defaultdict(list)
    for work in db.query(Work).all():
        if state and work.state != state:
            continue
